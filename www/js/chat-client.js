$(function() {
    ...
    socket.on('system', (user)=> { 
      ...
      $('#messages').append(`<p class='system'><span>${data}</span><br /><span>${user.name}  ${user.status}chatroom<span></p>`);
      // 滚动条总是在最底部
    ...
    socket.on('shake', (user)=> { 
      var data = new Date().toTimeString().substr(0, 8);
      $('#messages').append(`<p class='system'><span>${data}</span><br /><span>${user.name}send a notification</span></p>`);
      shake();
      // 滚动条总是在最底部
      $('#messages').scrollTop($('#messages')[0].scrollHeight);
    });
